...
MimePartStream.prototype.setDisposition = function (disposition) {
  if (disposition) {
    this._addHeader('Content-Disposition', disposition);
  }
};

MimePartStream.prototype.setTransferEncoding = function (transferEncoding) {
  this._addHeader('Content-Transfer-Encoding', transferEncoding);
...
MimePartStream.prototype._addHeader = function (key, value) {
...
MimePartStream.prototype._pushHeaders = function () {
  ...
  this._pushEnabled = this.push(this._headers.toString());
};
...
function mimePartStream(options) {
  ...
  stream.setDisposition(options.disposition);
